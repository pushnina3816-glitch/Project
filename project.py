...
frog_image = pygame.image.load("frog2.jpg").convert()


fly_original = pygame.image.load("fly.jpg").convert_alpha()
fly_image = pygame.transform.scale(fly_original, (15, 10))

obstacle_original = pygame.image.load("stick.jpg").convert_alpha()
...
while running:
            ...
            if game_state == "playing":
                if event.key == pygame.K_w:
                    ...
                    projectiles.append({
                        'x': screen_width // 2,
                        'y': y,
                        'vx': projectile_power * math.cos(rad),
                        'vy': -projectile_power * math.sin(rad),
                        'active': True
                    })
    ...
    if game_state == "playing":
        ...
        for fly in flies:
            ...
            if -20 <= fly['x'] <= screen_width + 20:
                ...
                if frog_rect.colliderect(fly_rect):
                    ...
                    screen.blit(fly_image, (fly['x'] - 7, fly['y'] - 5))
        ...
        for proj in projectiles:
            if proj['active']:
                proj['vy'] += projectile_gravity
                proj['x'] += proj['vx']
                proj['y'] += proj['vy']
                
                if (proj['x'] < -10 or proj['x'] > screen_width + 10 or 
                    proj['y'] < -10 or proj['y'] > screen_height + 10):
                    proj['active'] = False
                    continue
                ...
                if not hit_fly:
                    screen.blit(projectile_image, (proj['x'] - 5, proj['y'] - 5))
        ...
        if obstacle_timer >= OBSTACLE_SPAWN_INTERVAL:
            ...
            obstacles.append({
                ...
                'speed': x_speed if x_speed != 0 else (3 if obstacle_x > screen_width else -3),
                'color': (139, 69, 19),
                'image': obstacle_img
            })
        ...
        for obstacle in obstacles:
            ...
            if -50 <= obstacle['x'] <= screen_width + 50:
        ...
        if on_ground:
            ...
            temp_x, temp_y = screen_width // 2, y
